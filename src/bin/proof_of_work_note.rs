...
async fn find_hash(input: Word, target: u64) -> Word {
    ...
    loop {
        ...
        if val < target {
            println!("Solution DIGEST: {:?}", digest);
            println!("input: {:?}", input);
...
async fn main() -> Result<(), ClientError> {
    ...
    let target_difficulty: u64 = 35184372088832;

    let assembler: Assembler = TransactionKernel::assembler().with_debug_mode(true);
    let file_path = Path::new("./masm/notes/proof_of_work_note.masm");
    ...
    println!("finding nonce input");
    let solution = find_hash(pow_input, target_difficulty).await;
    println!("solution found: {:?}", solution);

    let tx_note_consume_request = TransactionRequestBuilder::new()
        .with_unauthenticated_input_notes([(increment_note, Some(solution))])
        .build();
